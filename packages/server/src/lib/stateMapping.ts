 ...
 * (`pipelineTools.ts`). Kept dependency-free (only `json-logic-js`) so it can be
 * imported by `pipelineTools.ts` without pulling in the heavier orchestration
 * import graph — importing the helpers from `orchestrationNodeExecutors` instead
 * would create the cycle `orchestrationNodeExecutors → tools → pipelineTools →
 * orchestrationNodeExecutors`, which perturbs module-init order and breaks
 * orchestration runs at runtime.
 */
