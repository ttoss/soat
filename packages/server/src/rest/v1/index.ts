import { Router } from '@ttoss/http-server';

import { activityRouter } from './activity';
import { actorsRouter } from './actors';
import { actorTagsRouter } from './actorTags';
import { agentsRouter } from './agents';
import { aiProvidersRouter } from './aiProviders';
import { apiKeysRouter } from './apiKeys';
import { approvalsRouter } from './approvals';
import { auditLogRouter } from './auditLog';
import { chainsRouter } from './chains';
import { chatsRouter } from './chats';
import { conversationsRouter } from './conversations';
import { documentsRouter } from './documents';
import { embeddingsRouter } from './embeddings';
import { evaluationsRouter } from './evaluations';
import { exceptionsRouter } from './exceptions';
import { filesRouter } from './files';
import { formationsRouter } from './formations';
import { generationsRouter } from './generations';
import { guardrailsRouter } from './guardrails';
import { ingestionRulesRouter } from './ingestionRules';
import { knowledgeRouter } from './knowledge';
import { memoriesRouter } from './memories';
import { memoryRulesRouter } from './memoryRules';
import { memoryStoresRouter } from './memoryStores';
import { metadataSchemasRouter } from './metadataSchemas';
import { modelRoutesRouter } from './modelRoutes';
import { oauthRouter } from './oauth';
import { openapiRouter } from './openapi';
import { orchestrationQueueRouter } from './orchestrationQueue';
import { orchestrationsRouter } from './orchestrations';
import { policiesRouter } from './policies';
import { projectsRouter } from './projects';
import { quotasRouter } from './quotas';
import { secretsRouter } from './secrets';
import { sessionsRouter } from './sessions';
import { tasksRouter } from './tasks';
import { toolsRouter } from './tools';
import { tracesRouter } from './traces';
import { triggersRouter } from './triggers';
import { usageRouter } from './usage';
import { usersRouter } from './users';
import { webhooksRouter } from './webhooks';
import { workflowsRouter } from './workflows';

const v1Router = new Router();

v1Router.use(activityRouter.routes());
v1Router.use(formationsRouter.routes());
v1Router.use(generationsRouter.routes());
v1Router.use(guardrailsRouter.routes());
v1Router.use(agentsRouter.routes());
v1Router.use(sessionsRouter.routes());
v1Router.use(chainsRouter.routes());
v1Router.use(chatsRouter.routes());
v1Router.use(actorsRouter.routes());
v1Router.use(actorTagsRouter.routes());
v1Router.use(aiProvidersRouter.routes());
v1Router.use(apiKeysRouter.routes());
v1Router.use(approvalsRouter.routes());
v1Router.use(evaluationsRouter.routes());
v1Router.use(exceptionsRouter.routes());
v1Router.use(auditLogRouter.routes());
v1Router.use(memoryStoresRouter.routes());
v1Router.use(memoriesRouter.routes());
v1Router.use(memoryRulesRouter.routes());
v1Router.use(modelRoutesRouter.routes());
v1Router.use(oauthRouter.routes());
v1Router.use(policiesRouter.routes());
v1Router.use(conversationsRouter.routes());
v1Router.use(documentsRouter.routes());
v1Router.use(ingestionRulesRouter.routes());
v1Router.use(metadataSchemasRouter.routes());
v1Router.use(embeddingsRouter.routes());
v1Router.use(filesRouter.routes());
v1Router.use(knowledgeRouter.routes());
v1Router.use(projectsRouter.routes());
v1Router.use(quotasRouter.routes());
v1Router.use(secretsRouter.routes());
v1Router.use(orchestrationQueueRouter.routes());
v1Router.use(orchestrationsRouter.routes());
v1Router.use(workflowsRouter.routes());
v1Router.use(tasksRouter.routes());
v1Router.use(toolsRouter.routes());
v1Router.use(tracesRouter.routes());
v1Router.use(triggersRouter.routes());
v1Router.use(usageRouter.routes());
v1Router.use(usersRouter.routes());
v1Router.use(webhooksRouter.routes());
v1Router.use(openapiRouter.routes());

export { v1Router };
