 ...
 *
 * Both denial forms count: a manual `ctx.status = 403` and
 * `throw new DomainError('FORBIDDEN', …)`. Matching one alone would leave the
 * check passing vacuously — green because the spelling it hunts for appears
 * nowhere, not because no route hand-rolls the gate.
 */
