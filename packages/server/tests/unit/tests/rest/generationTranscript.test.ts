...
describe('GET /api/v1/generations/:generation_id/transcript', () => {
  ...
  // #1024: a reused `trace_id` groups generations, so the trace's steps object
  // holds several turns. A transcript is scoped to one turn and must take its
  // own slice rather than report the whole object.
  test('stays scoped to its own turn when the trace groups two generations', async () => {
    const first = await runGeneration({ content: 'First grouped question?' });

    stubResponses = [textCompletion('Second grouped answer.')];
    const second = await runGeneration({
      content: 'Second grouped question?',
      traceId: first.trace_id,
    });
    expect(second.trace_id).toBe(first.trace_id);

    const firstRes = await transcript(first.id);
    const secondRes = await transcript(second.id);
    expect(firstRes.status).toBe(200);
    expect(secondRes.status).toBe(200);

    // Each reads back only its own answer, and its own counter.
    expect(firstRes.body.output.content).toBe(ASSISTANT_TEXT);
    expect(secondRes.body.output.content).toBe('Second grouped answer.');
    expect(firstRes.body.steps).toHaveLength(1);
    expect(secondRes.body.steps).toHaveLength(1);
    expect(firstRes.body.step_count).toBe(1);
    expect(secondRes.body.step_count).toBe(1);

    // ...while the trace counts both, which is what grouping is for.
