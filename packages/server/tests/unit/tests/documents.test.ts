...
describe('Documents', () => {
  ...
  describe('resolveDocumentQuery — boundaryPolicy filtering', () => {
    let boundaryProjectId: string;
    let boundaryProjectInternalId: number;
    ...
    beforeAll(async () => {
      ...
      const projectRes = await authenticatedTestClient(adminToken)
        .post('/api/v1/projects')
        .send({ name: 'Boundary Policy Project' });
      boundaryProjectId = projectRes.body.id;

      const policyRes = await authenticatedTestClient(adminToken)
        .post(`/api/v1/projects/${boundaryProjectId}/policies`)
        .send({
          permissions: [
            'documents:CreateDocument',
            'documents:SearchDocuments',
      ...
      await authenticatedTestClient(adminToken)
        .post(`/api/v1/projects/${boundaryProjectId}/members`)
        .send({ user_id: userId, policy_id: policyRes.body.id });

      const docAllowed = await authenticatedTestClient(userToken)
        .post('/api/v1/documents')
        .send({
          project_id: boundaryProjectId,
          content: 'Allowed boundary document.',
          filename: 'boundary-allowed.txt',
        });
      allowedDocId = docAllowed.body.id;

      const docDenied = await authenticatedTestClient(userToken)
        .post('/api/v1/documents')
        .send({
          project_id: boundaryProjectId,
          content: 'Denied boundary document.',
          filename: 'boundary-denied.txt',
        });
      deniedDocId = docDenied.body.id;

      // Look up internal project id for direct lib call
      const { db } = await import('../../../src/db');
      const project = await db.Project.findOne({
        where: { publicId: boundaryProjectId },
      });
      boundaryProjectInternalId = project!.id;
    });
    ...
    test('boundaryPolicy allows only matching documents', async () => {
      ...
      expect(results[0].id).toBe(allowedDocId);
    });
    ...
    test('boundaryPolicy with wildcard allows all documents', async () => {
      ...
      expect(ids).toContain(deniedDocId);
    });
    ...
    test('no boundaryPolicy returns all documents unfiltered', async () => {
      const results = await resolveDocumentQuery({
        projectIds: [boundaryProjectInternalId],
        config: {
          documentIds: [allowedDocId, deniedDocId],
