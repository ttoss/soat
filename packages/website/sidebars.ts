import type { SidebarsConfig } from '@docusaurus/plugin-content-docs';

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

/**
 * Navbar structure: Docs · Tutorials · Reference (see README.md).
 *
 * Files are grouped explicitly (not autogenerated) so the sidebar mirrors the
 * mental model taught in introduction.md and getting-started/concepts.md — a
 * new module or tutorial page needs its doc id added to the matching category.
 *
 * Getting Started stays short: only what a reader needs before their first
 * call. A page a newcomer does not need, or that is only linked from module
 * deep-dives, belongs under Advanced instead.
 */
const sidebars: SidebarsConfig = {
  docsSidebar: [
    'introduction',
    'agent-system-layers',
    {
      type: 'category',
      label: 'Getting Started',
      link: { type: 'doc', id: 'getting-started/quick-start' },
      collapsed: false,
      items: ['getting-started/quick-start', 'getting-started/concepts'],
    },
    {
      type: 'category',
      label: 'Client Surfaces',
      link: { type: 'doc', id: 'client-surfaces' },
      items: [
        'client-surfaces',
        {
          type: 'category',
          label: 'SDK',
          link: { type: 'doc', id: 'sdk/introduction' },
          items: ['sdk/introduction', 'sdk/usage'],
        },
        {
          type: 'category',
          label: 'CLI',
          link: { type: 'doc', id: 'cli/introduction' },
          items: ['cli/introduction', 'cli/usage'],
        },
        {
          type: 'category',
          label: 'MCP',
          link: { type: 'doc', id: 'mcp/introduction' },
          items: ['mcp/introduction', 'mcp/connecting'],
        },
      ],
    },
    {
      type: 'category',
      label: 'Platform',
      link: { type: 'doc', id: 'modules/index' },
      collapsed: false,
      items: [
        {
          type: 'category',
          label: 'Identity & Access',
          items: [
            'modules/users',
            'modules/projects',
            'modules/iam',
            'modules/policies',
            'modules/api-keys',
            'modules/oauth',
          ],
        },
        {
          type: 'category',
          label: 'Storage & Retrieval',
          items: [
            'modules/files',
            'modules/documents',
            'modules/embeddings',
            'modules/ingestion-rules',
            'modules/knowledge',
            'modules/memories',
          ],
        },
        {
          type: 'category',
          label: 'Agents & Conversations',
          items: [
            'modules/ai-providers',
            'modules/model-routes',
            'modules/agents',
            'modules/tools',
            'modules/sessions',
            'modules/conversations',
            'modules/chats',
            'modules/actors',
            'modules/generations',
          ],
        },
        {
          type: 'category',
          label: 'Orchestration & Automation',
          items: [
            'modules/orchestrations',
            'modules/workflows',
            'modules/triggers',
            'modules/webhooks',
            'modules/approvals',
            'modules/guardrails',
          ],
        },
        {
          type: 'category',
          label: 'Declarative Deployment',
          items: ['modules/formations'],
        },
        {
          type: 'category',
          label: 'Improvement & Adaptation',
          items: ['modules/evaluations'],
        },
        {
          type: 'category',
          label: 'Operations',
          items: [
            'modules/traces',
            'modules/audit-log',
            'modules/exceptions',
            'modules/activity',
            'modules/usage',
            'modules/quotas',
            'modules/secrets',
            'modules/docs',
          ],
        },
      ],
    },
    {
      type: 'category',
      label: 'Advanced',
      items: [
        'advanced/choosing-an-automation-model',
        'advanced/engines-and-algorithms',
        'advanced/building-on-soat',
        'advanced/expressions-and-templating',
        'advanced/sync-and-async',
        'advanced/memory-and-knowledge-engine',
        'advanced/tool-context',
      ],
    },
    {
      type: 'category',
      label: 'Self-Hosting & Production',
      link: { type: 'doc', id: 'self-hosting/configuration' },
      items: ['self-hosting/configuration'],
    },
  ],

  tutorialSidebar: [
    'tutorials/index',
    {
      type: 'category',
      label: 'Basics',
      collapsed: false,
      items: [
        'tutorials/permissions',
        'tutorials/chat-with-llm',
        'tutorials/connect-third-party-llms',
      ],
    },
    {
      type: 'category',
      label: 'Agents',
      items: [
        'tutorials/agent-soat-tools',
        'tutorials/client-tools',
        'tutorials/memories-agent',
        'tutorials/debug-session-generation-trace-history',
        'tutorials/call-aws-and-gcp-apis-from-an-agent',
      ],
    },
    {
      type: 'category',
      label: 'Knowledge & RAG',
      items: [
        'tutorials/embeddings',
        'tutorials/agent-with-pdfs',
        'tutorials/ingest-images-and-audio',
      ],
    },
    {
      type: 'category',
      label: 'Orchestration & Multi-Agent',
      items: [
        'tutorials/orchestrate-a-sonnet',
        'tutorials/orchestrate-a-sonnet-with-workflows',
        'tutorials/multi-agent-orchestration',
        'tutorials/conditional-orchestration',
        'tutorials/orchestration-control-flow',
        'tutorials/approval-gate',
        'tutorials/create-an-agent-squad',
        'tutorials/close-the-monthly-books',
      ],
    },
    {
      type: 'category',
      label: 'Automation & Deployment',
      items: [
        'tutorials/formations',
        'tutorials/automate-a-flow-with-triggers',
        'tutorials/agent-versioning-and-canary-rollout',
      ],
    },
    {
      type: 'category',
      label: 'Evaluation & Quality',
      items: [
        'tutorials/evaluate-an-agent',
        'tutorials/judge-open-ended-answers',
        'tutorials/gate-a-canary-promotion-on-an-eval',
        'tutorials/replay-a-bad-turn',
      ],
    },
    {
      type: 'category',
      label: 'Governance & Safety',
      items: [
        'tutorials/agent-boundary-policy',
        'tutorials/build-an-agent-harness',
        'tutorials/gate-a-tool-with-guardrails',
        'tutorials/per-user-credentials-with-tool-context',
        'tutorials/data-retention-and-zero-retention',
      ],
    },
    {
      type: 'category',
      label: 'Cost & Operations',
      items: [
        'tutorials/metering-and-budgets',
        'tutorials/cap-spend-per-end-user',
      ],
    },
  ],

  // Generated / lookup-oriented material. Most doc ids below only exist after
  // `pnpm generate-all` runs (they are gitignored build outputs).
  referenceSidebar: [
    {
      type: 'category',
      label: 'REST API',
      link: { type: 'doc', id: 'api/index' },
      collapsed: false,
      items: [{ type: 'autogenerated', dirName: 'api' }],
    },
    {
      type: 'category',
      label: 'SDK Services',
      link: { type: 'doc', id: 'sdk/services' },
      items: [{ type: 'autogenerated', dirName: 'sdk/services' }],
    },
    {
      type: 'category',
      label: 'CLI Commands',
      link: { type: 'doc', id: 'cli/commands' },
      items: [{ type: 'autogenerated', dirName: 'cli/commands' }],
    },
    {
      type: 'category',
      label: 'MCP Tools',
      link: { type: 'doc', id: 'mcp/tools' },
      items: [{ type: 'autogenerated', dirName: 'mcp/tools' }],
    },
    {
      type: 'category',
      label: 'Formation Types',
      link: { type: 'doc', id: 'formations-types/index' },
      items: [{ type: 'autogenerated', dirName: 'formations-types' }],
    },
    'permissions',
    'webhook-events',
    'error-codes',
    'openapi-specs',
  ],
};

export default sidebars;
