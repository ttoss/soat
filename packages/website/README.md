...
- **Docs** (`docsSidebar`) — learning-oriented content:
  ...
  - **Advanced** — cross-cutting platform behavior that is not tied to a single module (`docs/advanced/*.md`): choosing an automation model, engines & algorithms, building on SOAT, expressions & templating, synchronous & asynchronous execution, concurrent writes, the project as a single source of truth, the memory & knowledge engine, and tool context.
