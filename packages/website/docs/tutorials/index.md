...
- **Locking an agent down?** [Bound an Agent with a Boundary Policy](/docs/tutorials/agent-boundary-policy), [Tag-Based Access Control](/docs/tutorials/tag-based-access-control) to grant by attribute, then [Guardrails](/docs/tutorials/gate-a-tool-with-guardrails) for the tool types a boundary cannot reach.
- **Retrieval and RAG?** [Embeddings](/docs/tutorials/embeddings), an [agent over a library of PDFs](/docs/tutorials/agent-with-pdfs), and [ingest images and audio](/docs/tutorials/ingest-images-and-audio); then [measure retrieval quality](/docs/tutorials/measure-retrieval-quality) with recall@k and MRR on your own corpus.
- **Many agents writing one corpus?** [Govern Metadata with Schemas](/docs/tutorials/govern-metadata-with-schemas) for what a report must say, then [Single Source of Truth](/docs/tutorials/single-source-of-truth): conditional writes, versions, withdrawal, retraction, relations and export.
