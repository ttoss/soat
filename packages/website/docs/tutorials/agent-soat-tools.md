...
- An Ollama instance accessible at `http://ollama:11434` with model `qwen3.5:0.8b` pulled (`ollama pull qwen3.5:0.8b`).
...
PROVIDER_ID=$(curl -s -X POST "$SOAT_BASE_URL/api/v1/ai-providers" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"project_id\":\"$PROJECT_ID\",\"name\":\"Ollama\",\"provider\":\"ollama\",\"default_model\":\"qwen3.5:0.8b\"}" | jq -r '.id')
echo "Provider: $PROVIDER_ID"
```
