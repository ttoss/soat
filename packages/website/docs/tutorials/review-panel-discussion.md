...
- SOAT running locally with Ollama. Follow the [Quick Start](/docs/getting-started) guide, and skim [Key Concepts](/docs/getting-started/concepts) if SOAT's mental model is new to you. For production hardening (secrets, env vars), see [Configuration](/docs/self-hosting/configuration).
