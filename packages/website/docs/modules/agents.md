...
| `min_score`       | `number`              | Minimum raw cosine similarity (0–1) a vector candidate must reach to be ranked (default: 0.5). The same floor the search endpoint spells `min_similarity` — see [Knowledge — Relevance knobs](./knowledge.md#relevance-knobs) |
