...
`knowledge_config` in the [`POST /agents/{agent_id}/generate`](/docs/api/agents/create-agent-generation) body overrides the stored config for one call: `memory_store_ids`, `document_ids` and `document_paths` are unioned with the stored arrays; `tags` pairs are merged, the override winning per key; `min_score` and `limit` take the per-generation value. `write_memory_store_id` is agent-level only; `write_memory` tool: [Memories](./memories.md#agent-integration).
