...
- **`type`** — one of: `ai_provider`, `tool`, `agent`, `actor`, `api_key`, `chat`, `conversation`, `document`, `file`, `ingestion_rule`, `memory`, `memory_entry`, `orchestration`, `policy`, `project_price`, `secret`, `session`, `webhook`. See [Formations Types](/docs/formations-types) for the full properties reference.
