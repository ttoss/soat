...
A run whose graph contains a `loop` or `sub_orchestration` node is covered by the receipt only for its **own** nodes: those nodes start child runs, whose events are attributed to the child, so the parent's receipt shows the starting node's execution cost and not what the children spent. That is deliberate — the line items carry a `node_id`, and merging a child's nodes in would mix node ids from two graphs under one list. For the delegated total read `usage_including_nested` on the run, and for the children themselves the `parent_orchestration_run_id` filter; both are described in [Run usage](./orchestrations.md#run-usage).
