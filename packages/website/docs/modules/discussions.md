...
A **Discussion** is the home of deep thinking in SOAT. It is a reusable configuration (who deliberates and how); each invocation is a **DiscussionRun** (what was deliberated and what came out) — the same split as an [Agent](./agents.md) and its generations. Author a discussion once, then invoke it many times: standalone (brainstorming, red-teaming, expert review) or, more commonly, from an agent mid-loop via a [`soat` tool](./tools.md#soat) bound to `create-discussion-run`.
...
An agent thinks by attaching a [`soat` tool](./tools.md#soat) bound to the `create-discussion-run` action, with the discussion pinned via `preset_parameters`:
...
```json
{
  ...
  "preset_parameters": { "discussion_id": "disc_V1StGXR8Z5jdHi6B" }
}
```

`preset_parameters` strips `discussion_id` from the schema the model sees, so the model supplies only a `topic`. The server runs the discussion synchronously and returns the completed run as the tool result. Use `tool_choice: required` or a step rule to force "discuss before acting".

Because the call goes through the REST route, it is authorized like any other: the invoking credential needs `discussions:CreateDiscussionRun`, and the agent's boundary policy must allow the action.
...
Invoke the discussion from the agent via a `soat` tool bound to `create-discussion-run`, or call `POST /discussions/{id}/runs` before/after a generation. No data migration is required — the old `reasoning` config simply stops being read.
