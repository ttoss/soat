...
- **Custom retrieval composition.** Exact-term matching needs no index of your own — the lexical channel is built in, and a recency decay on memory results is one knob (`recency_half_life_days`). For reranking, or fusion with a signal the engine does not have, call [`POST /api/v1/knowledge/search`](/docs/api/knowledge/search-knowledge) with a generous `limit`, re-rank on your side using `similarity_score` plus your own features, and pass the survivors as input messages.
