...
SOAT has two ways to automate work that spans more than one step, with different
topologies and lifetimes:

- An **[orchestration](/docs/modules/orchestrations) is a pipeline that _ends_** — a
  directed acyclic graph that flows forward through its nodes and terminates.
...
The third answer is the most common one:
...
negotiating`, a kanban card dragged back a column: none fit a DAG. Neither module is a
subset of the other.

## Step 0 — you may need neither

Both models are the **graph** layer, the layer to build last ([The Layers of an Agent
System](/docs/agent-system-layers)). Reach for a single [agent](/docs/modules/agents)
first; three dials bound its loop:
...
Escalate to a graph only when the work needs something an agent loop cannot express:
...
An agent is a [node type](/docs/modules/orchestrations#node-types), so a graph wraps the
loop you already have rather than replacing it.

## What starts them

Both are started by a client, or by a [trigger](/docs/modules/triggers):
...
- An **`event` trigger** subscribes to an internal platform event
  ...
  `emit_event`). Delivery is best-effort and unordered, and the reactive edge is capped by
  a [causation depth guard](/docs/modules/triggers#loops-and-cost). Use it when the work is
  a *reaction* and promptness is the point.
- A **`schedule` trigger** runs on a cron cadence and is recovered from the database: a
  firing missed while the server was down is coalesced into one catch-up. Use it when the
  work is periodic, must not be dropped, or as the backstop under an event trigger whose
...
to a [`builtin` tool](/docs/modules/tools#builtin) for `create-task` or `transition-task`.
There is no dedicated node type:
...
A dispatched **agent** does the same with a `builtin` tool of its own.

Both kinds of dispatch act as the principal that started the chain (the person or key that
created the task or fired the transition), and each automated hop inherits that identity.
See [Run identity](/docs/modules/orchestrations#durable-background-execution).

### Two rules for the composed edge

**Keep the orchestration → workflow edge fire-and-forget.** A run is bounded and holds a
lease and a queue slot; a task lives for days and can move backward. Let the run end and
the task's own state machine carry things forward.

**Bound the cycle yourself.** A state whose orchestration transitions the task back into
that same state is a cycle no validator sees (cycle detection is per-graph; workflow cycles
are deliberate). The task engine's
[automation chain budget](/docs/modules/workflows#the-automation-chain-budget) refuses the
hop that would exceed it; reaching it means a loop is running unattended and deserves an
alert.
...
- **Declarative deployment** — both are [formation](/docs/modules/formations) resource
  ...
  graphs call deploy as one stack.
