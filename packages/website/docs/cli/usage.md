...
- `--secret` — verify the payload's signature against this webhook/trigger secret, read from `X-Soat-Signature` (`t=<unix>,v1=<hex>` on a webhook delivery, `sha256=<hex>` on a trigger payload); mismatch is rejected with `401`
