...
export class Task extends Model {
  ...
  // tasks block deletion" semantics.
  @BelongsTo(
    () => {
      return Workflow;
    },
  ...
  // re-shape a task already in flight. Every read of the state machine
  ...
  // author's declared `payload_writes`.
  ...
  // a namespace callers cannot write, unlike the payload bag.
  @Column({ type: DataType.JSONB, allowNull: true })
  declare lastResult: unknown;

  // Forwarded as `X-Soat-Context-*` headers on the next dispatch's tool calls,
  // replaced wholesale by each move that supplies one.
  //
  ...
  // Separate from `payload`, which the engine writes into and guards
  ...
  // intervention — a state can dispatch work that transitions straight
  ...
  // chain continues, which is the only spend a task drives on its own.
