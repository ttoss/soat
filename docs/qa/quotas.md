...
- [x] In-flight generation is not killed (overshoot bounded to one generation)
...
- [x] Same key still `200` on an action it *is* granted (proves the key is otherwise live)
...
- [ ] **Multi-replica counter atomicity** (the atomic `UPDATE … RETURNING` claim) — needs a multi-replica deployment; only a single instance was reachable in both passes.
- [ ] **Real-time rollover for `rolling_1h` / `rolling_24h` / `calendar_month`** — would require waiting out the actual window. Only window-key format and `resets_at` were checked. `rolling_1m` rollover *was* observed for real.
