...
| 2.2 | Tool-boundary interceptor: classify → route | **Foundation shipped; live dispatch wiring is the follow-up.** Landed: (a) the pure **evaluation engine** (`lib/guardrailEvaluation.ts`) — evaluate the `class` expression with **fail-closed default class C** on any invalid result (`null`, typo, non-class); A→execute, D→blocked, C→route_to_approval, B→execute iff its guard passes else `tripwire` (or `route_to_approval` with `escalate`); **stricter-wins composition** across all applying guardrails (`blocked` > `tripwire` > `route_to_approval` > `execute`), with guards-AND falling out of the ordering; (b) the **`guardrail_ids` attach layer** on the project (baseline), agent (whole surface), and tool (every agent) — model columns, lib/REST/OpenAPI/formations sync, referenced-id existence validation, and the **attach-cheap / detach-gated** asymmetry (adding needs only the resource's update permission; removing at any scope requires `guardrails:DetachGuardrail`); (c) the **`DELETE`→`409`-while-referenced** guard + dangling-reference scan (closes the 2.1 deferral). **Deferred to the immediate follow-up (folds in with 2.3):** wiring the engine into the tool-dispatch path (`agentToolResolver` gate → C files the approval via M1's return-pending/continuation/dedup, D returns a blocked tool result, tripwire aborts), threading `guardrail_context`, `soat.*` population, and the `requires_action`-handoff gate for client tools |
