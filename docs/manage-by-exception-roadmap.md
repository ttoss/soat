...
| 2.2 ✅ | Tool-boundary interceptor: classify → route | **Shipped** (foundation in #620; live dispatch wiring folded in with 2.3). Foundation: (a) the pure **evaluation engine** (`lib/guardrailEvaluation.ts`) — evaluate the `class` expression with **fail-closed default class C** on any invalid result (`null`, typo, non-class); A→execute, D→blocked, C→route_to_approval, B→execute iff its guard passes else `tripwire` (or `route_to_approval` with `escalate`); **stricter-wins composition** across all applying guardrails (`blocked` > `tripwire` > `route_to_approval` > `execute`), with guards-AND falling out of the ordering; (b) the **`guardrail_ids` attach layer** on the project (baseline), agent (whole surface), and tool (every agent) — model columns, lib/REST/OpenAPI/formations sync, referenced-id existence validation, and the **attach-cheap / detach-gated** asymmetry; (c) the **`DELETE`→`409`-while-referenced** guard + dangling-reference scan (closes the 2.1 deferral). Follow-up (this change): the engine is now wired into the tool-dispatch path — the `agentToolResolver` gate (`lib/agentToolGuardrail.ts`) collects the project/agent/tool guardrails (`lib/guardrailCollection.ts`), composes stricter-wins, and enacts: **C files the approval** via M1's return-pending/continuation/dedup (`origin: tool_call`), **D returns a blocked tool result**, a **tripwire aborts**, and A / passing-B execute. **Still deferred:** the `requires_action`-handoff gate for **client** tools (only server-executed tools are gated so far) and the orchestration tool-node dispatch path |
| 2.3 ✅ | Guard evaluation + guardrail context (`args.*` / `context.*` / `soat.*`) | **Shipped.** Reuses the orchestration JSON Logic evaluator (no LLM in the path). Context is **application-owned** (`lib/guardrailContext.ts`): the caller passes `guardrail_context` on the generation request (threaded through `createGeneration` → `buildGenerationContext`); an optional `context_tool_id` on the guardrail is called at evaluation time under the **calling agent's credentials** (via `callTool`, platform-initiated, bounded by a per-call timeout + short per-`(project, guardrail)` TTL cache) and combined per `context_mode` (`merge` default — tool wins; or `replace`). `soat.*` is populated for **only the referenced catalog keys**: identity/run synchronously, `soat.usage.*` from windowed usage-metering sums, `soat.activity.*` left unresolvable (fail-closed) until task 5.4. Fail-closed throughout: a missing `context.*` key, a context-tool failure/timeout, or an unresolvable `soat.*` resolves the class to `default_class` and fails the guard |
| 2.4 ✅ | Project attach scope (`guardrail_ids` on the project) | **Shipped** (attach columns in #620; runtime composition here). No override resource: a per-project tighter posture is a guardrail attached at the project scope, composed with agent/tool scopes by stricter-wins + guards-AND in the interceptor — tighten-only by construction. Detach at any scope requires `guardrails:DetachGuardrail`, so the floor can't be silently lowered |
| 2.5 ✅ | Tripwires + `escalate` on a failing class-B guard | **Shipped** (routing). A failing class-B guard aborts the action by default (a `tripwire` tool result — a hard, non-LLM stop); `escalate: true` routes the call to the approval queue instead. `escalate` is per-guardrail and the strictest decision still wins (`tripwire` outranks `route_to_approval`). **Deferred:** filing an `ExceptionItem` on a tripwire awaits the Exceptions backlog item; today the tripwire returns a structured aborted tool result |
| 2.6 ✅ | `guardrail_evaluation` audit record | **Shipped.** Every evaluation (execute / route-to-approval / block / tripwire) writes one append-only `GuardrailEvaluation` row (`gev_` prefix) per guardrail evaluated, fire-and-forget: governing `guardrail_version` (null for a dangling reference), the `scope` (`project` / `agent` / `tool`), resolved `class`, `decision`, `guard_result`, `context_source`, and a flat `context_snapshot` of **only the referenced vars** (fully-qualified `args.*` / `context.*` / `soat.*`), plus `agent_id` / `run_id` / `generation_id` provenance and the filed `approval_id`. **Note:** records live in their own table until the M5 activity feed exists to absorb them |
| 2.7 ✅ | Deprecate per-binding `approval_policy` | **Shipped.** Guardrails are now the **single** routing source: the generation entry points (`agentGenerationContext.ts`, `agentGenerationRecovery.ts`) no longer build an approval-gate context, so a live generation never routes on `approval_policy` — proven end-to-end through `buildGenerationContext` (a `require_approval` binding executes the tool and files no item). The field stays readable/writable for the deprecation window (write validation in `agentToolBindings.ts` and the derived echo are untouched) and is marked `deprecated` in the OpenAPI `ToolBinding` / `ToolApprovalPolicy` schemas + formations `tool_bindings` description; docs (agents/tools/approvals/guardrails) state it is no longer enforced, with a `{ "class": "C" }` guardrail as the migration path. The M1 dispatch machinery (`gateResolvedTools`, `buildResolverApprovalContext`, the resolver `approval` param) is retained as deprecation-window code, removed in 2.8 |
| 2.8 ✅ | Remove `approval_policy` completely (breaking) | **Shipped (breaking).** The per-binding `approval_policy` is gone: the `AgentToolBinding` field and its whole validation surface (shape / rules / effect / expiry / client-binding / referenced-tool checks) are deleted from `agentToolBindings.ts`; the dispatch-path evaluation branch (`gateResolvedTools`, `buildResolverApprovalContext`, `evaluateApprovalEffect`, `policyCanRequireApproval`, the resolver `approval` param) is deleted from `agentToolApproval.ts` / `agentToolResolver.ts`; the OpenAPI `ToolApprovalPolicy` schema + `ToolBinding.approval_policy` property and the formations `tool_bindings` handling are removed; docs (the `agents.md` Approval Policy section + all `#approval-policy` links in `tools.md` / `approvals.md` / `guardrails.md`) are purged; the generated SDK/CLI drop it on regenerate. Because the field is gone from the schema, the strict-fields validator now **rejects** a stray `approval_policy` (`400 VALIDATION_FAILED`). Only M1's shared dispatch machinery (return-pending justification fields, dedup, resolved-action-name, default expiry) remains in `agentToolApproval.ts` — used **solely** by the guardrail interceptor |
| 2.9 ✅ | Dry-run evaluate endpoint (`POST /guardrails/{guardrail_id}/evaluate`) | **Shipped.** Runs the full pipeline (class expression, guard, context tool, live `soat.*`) over caller-supplied `args` / `guardrail_context` (+ optional `tool_id`) and returns the would-be `guardrail_evaluation` record; nothing executes, no approval item is filed, no audit row is written. Gated by `guardrails:EvaluateGuardrail`; on the generated SDK/CLI/MCP surface |
