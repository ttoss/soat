...
| G5 | Billing-grade cost metering             | Per-run token/cost accounting, idempotent under retries             | Per-**generation** metering + versioned price book **exist** (`UsageMeter`/`PriceBook`, receipt endpoint); per-**run** attribution/roll-up, aggregation, and non-LLM meter types (compute/storage/requests) do not — see [prd-usage-metering.md](./prd-usage-metering.md) |
